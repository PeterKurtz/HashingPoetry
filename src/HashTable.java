...
public class HashTable<K, E> {
     ...
     */
    public boolean insert(K key, E value) {
        // Insert x as active
        ...
        storage[currentPos] = new HashEntry<K, E>(key, value,true);
    ...
    public String toString(int limit) {
        ...
        for (int i = 0; i < storage.length && count < limit; i++) {
            if (storage[i] != null && storage[i].isActive) {
                sb.append(String.format("%d: %s", i, storage[i].key));
                sb.append("["+storage[i].value+"]\n");
                count++;
            }
        }
        return sb.toString();
     ...
     */
    private void rehash() {
        HashEntry<K, E>[] oldArray = storage;
        ...
        for (var entry : oldArray) {
            if (entry != null && entry.isActive) {
                insert(entry.key, (E) entry.value);
            }
        }
    }
    ...
    private class HashEntry<K, E> {
        public K key;   // the element
        public E value;
        public boolean isActive;  // false if marked deleted

        public HashEntry(K key, E value, boolean active) {
            this.key = key;
            this.value = value;
    ...
    private HashEntry<K, E>[] storage; // The array of elements
